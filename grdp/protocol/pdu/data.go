...
// ////////////////////////////////////////////////////////////////////////////
type AkuaFileTransferHead struct {
	FileNameLength uint16 `struc:"little"`
	FilePathLength uint16 `struc:"little"`
	FileName       []byte `struc:"sizefrom=FileNameLength"`
	FilePath       []byte `struc:"sizefrom=FilePathLength"`
}

func (p AkuaFileTransferHead) Key() string {
	return fmt.Sprintf("%s:%s", p.FileName, p.FilePath)
}

func NewAkuaFileTransferHead(fileName, filePath string) AkuaFileTransferHead {
	return AkuaFileTransferHead{
...
type AkuaDataPDU interface {
	Type2() uint8
	Unpack(io.Reader) error
	StreamId() uint8
}

func NewAkuaDataPDU(data AkuaDataPDU, shareId uint32) *DataPDU {
	dataBuff := &bytes.Buffer{}
	struc.Pack(dataBuff, data)
	return &DataPDU{
		Header: &ShareDataHeader{
			...
			UncompressedLength: uint16(len(dataBuff.Bytes()) + 4),
		},
		Data: data,
	}
}

type AkuaFileTransferStart struct {
	Head     AkuaFileTransferHead
	FileSize uint32 `struc:"little"`
}

func NewAkuaFileTransferStart(head AkuaFileTransferHead, fileSize uint32) *AkuaFileTransferStart {
	return &AkuaFileTransferStart{
		Head:     head,
		FileSize: fileSize,
	}
}
func (*AkuaFileTransferStart) Type2() uint8 {
	return 0x36
}
func (*AkuaFileTransferStart) Unpack(io.Reader) error {
	return nil
}
func (*AkuaFileTransferStart) StreamId() uint8 {
	return 0x05
}

type AkuaFileTransferAbort struct {
	Head AkuaFileTransferHead
}

func NewAkuaFileTransferAbort(head AkuaFileTransferHead) *AkuaFileTransferAbort {
	return &AkuaFileTransferAbort{
		Head: head,
	}
}
func (*AkuaFileTransferAbort) Type2() uint8 {
	return 0x27
}
func (*AkuaFileTransferAbort) Unpack(io.Reader) error {
	return nil
}
func (*AkuaFileTransferAbort) StreamId() uint8 {
	return 0x05
}

type AkuaFileTransferPacket struct {
	Head            AkuaFileTransferHead
	StartIndex      uint32 `struc:"little"`
	SlicingSize     uint32 `struc:"little"`
	FileDataSlicing []byte `struc:"sizefrom=SlicingSize"`
}

func NewAkuaFileTransferPacket(head AkuaFileTransferHead, startIndex uint32, dataSlicing []byte) *AkuaFileTransferPacket {
	return &AkuaFileTransferPacket{
		Head:            head,
		StartIndex:      startIndex,
		SlicingSize:     uint32(len(dataSlicing)),
		FileDataSlicing: dataSlicing,
	}
}
func (*AkuaFileTransferPacket) Type2() uint8 {
	return 0x21
}
func (*AkuaFileTransferPacket) Unpack(io.Reader) error {
	return nil
}
func (*AkuaFileTransferPacket) StreamId() uint8 {
	return 0x05
}

type AkuaFileTransferVerify struct {
	Head     AkuaFileTransferHead
	FileSize uint32 `struc:"little"`
	CRC32    uint32 `struc:"little"`
}

func NewAkuaFileTransferVerify(head AkuaFileTransferHead, fileSize, crc32 uint32) *AkuaFileTransferVerify {
	return &AkuaFileTransferVerify{
		Head:     head,
		FileSize: fileSize,
		CRC32:    crc32,
	}
}
func (*AkuaFileTransferVerify) Type2() uint8 {
	return 0x02
}
func (*AkuaFileTransferVerify) Unpack(io.Reader) error {
	return nil
}
func (*AkuaFileTransferVerify) StreamId() uint8 {
	return 0x05
}

type AkuaClientInputEventPDU struct {
	NumEvents           uint16               `struc:"little,sizeof=SlowPathInputEvents"`
	Pad2Octets          uint16               `struc:"little"`
	SlowPathInputEvents []SlowPathInputEvent `struc:"little"`
}

func (*AkuaClientInputEventPDU) Type2() uint8 {
	return PDUTYPE2_INPUT
}

func (*AkuaClientInputEventPDU) Unpack(reader io.Reader) error {
	return nil
}

func (*AkuaClientInputEventPDU) StreamId() uint8 {
	return STREAM_LOW
}

func NewAkuaMouseEvent(flags, x, y int) *AkuaClientInputEventPDU {
	return NewAkuaMouseEventWithType(flags, x, y, INPUT_EVENT_MOUSE)
}
func NewAkuaMouseEventWithType(flags, x, y, typ int) *AkuaClientInputEventPDU {
	p := &PointerEvent{
		PointerFlags: uint16(flags),
		XPos:         uint16(x),
		YPos:         uint16(y),
	}
	serial := p.Serialize()
	event := &AkuaClientInputEventPDU{
		NumEvents:  1,
		Pad2Octets: 0,
		SlowPathInputEvents: []SlowPathInputEvent{{
			EventTime:         0,
			MessageType:       uint16(typ),
			Size:              len(serial),
			SlowPathInputData: serial,
		}},
	}
	return event
}
func NewAkuaKeyboardEvent(button int, isPressed bool) *AkuaClientInputEventPDU {
	p := &ScancodeKeyEvent{
		KeyCode: uint16(button),
	}
	if !isPressed {
		p.KeyboardFlags |= KBDFLAGS_RELEASE
	}
	serial := p.Serialize()
	event := &AkuaClientInputEventPDU{
		NumEvents:  1,
		...
		SlowPathInputEvents: []SlowPathInputEvent{{
			EventTime:         0,
			MessageType:       INPUT_EVENT_SCANCODE,
			Size:              len(serial),
			SlowPathInputData: serial,
		}},
	}
...
type AkuaControlStatePacket struct {
	MagicCode1 uint16 `struc:"little"`
	State      uint8  `struc:"little"`
	MagicCode2 uint8  `struc:"little"`
}

func NewAkuaControlStatePacket(enable bool) *AkuaControlStatePacket {
	var state uint8 = 0x0
	if enable {
		state = 0xff
	}
	return &AkuaControlStatePacket{
		MagicCode1: 0x8135,
		State:      state,
		MagicCode2: 0xb3,
	}
}
func (*AkuaControlStatePacket) Type2() uint8 {
	return 0x02
}
func (*AkuaControlStatePacket) Unpack(io.Reader) error {
	return nil
}
